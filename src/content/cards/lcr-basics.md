...
LCR은 규제 하한이지 생존 보증이 아니다. 30일 유출률은 안정 소매예금 기본 5%(추가 요건과 관할 승인 시 3%), 운영성 예금 25%, 비금융기업 40%, 금융기관 예금 100%로 설계되어 있는데 [출처: basel-lcr40], 2023년 3월의 런은 이 30일 허용치를 하루 이틀 만에 소진했다. FSB 분석에 따르면 가장 빠른 3개 런의 일일 유출은 20~30%였고, 과거 1%/일 수준이던 중위값도 7%/일로 올라갔다 [출처: fsb-depositor-behaviour-2024]. 따라서 위기 대응 실무는 규제 LCR과 함께 "당일 런 렌즈"((현금 + 당일 담보차입 여력 − 최소 보유 현금) ÷ 예상 일일 유출 = 생존 일수)를 병행해 봐야 한다. 이 플랫폼의 대시보드가 두 렌즈를 나란히 표시하는 이유다.

또 하나의 함정은 적용 범위다. SVB는 2019년 tailoring 규칙으로 당시 LCR 의무 적용 대상이 아니었고, 내부 유동성 스트레스테스트(ILST)에서 4Q22 30일 부족이 반복되자 FHLB 차입을 늘리면서 가정을 완화했다 [출처: fed-svb-review-2023].
