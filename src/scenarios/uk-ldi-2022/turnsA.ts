  ...
  decisions: [
    {
      ...
      title: '위기 전 준비 패키지',
      prompt:
        '미니예산 발표 전, 이번 주에 무엇을 해 두시겠습니까? (최대 3개; A·D는 다른 항목과 함께 선택 불가)',
      context:
        '지금은 조용합니다. 준비 조치는 눈에 띄지 않지만, 다음 주의 모든 선택지는 오늘 담보와 운영 절차가 어떻게 준비되어 있는지에 달려 있습니다.',
      select: { min: 1, max: 3 },
      exclusive: [
        ['t0-a', 't0-b'],
        ['t0-a', 't0-c'],
      ...
      options: [
        ...
        },
        {
          ...
          description:
            '이번 주 주식을 매각(T+2)하고 금요일 컷오프 전에 풀에 현금 납입한다. 주식 비중이 20%→11%로 줄고 기대수익이 낮아진다. 실행가능성: 정상 시장에서 T+2 결제와 T+1 딜링으로 주내 완료 가능.',
          effects: [
