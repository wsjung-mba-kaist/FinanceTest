...
function EventBody({ e, fill }: { e: GameEvent; fill: (s: string) => string }) {
  switch (e.kind) {
    ...
    default:
      return <Markdown className="mt-1 text-[13px] leading-relaxed">{fill(e.body)}</Markdown>
  }
}
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {e?.correctionOf && <span className="text-muted">정정: {e.correctionOf}</span>}
        <span className="ml-auto flex items-center gap-1.5">
          {severityBadge(severity)}
          {time && <span className="num text-muted">{time}</span>}
        </span>
      </div>
      <h4 className="mt-1 text-[13px] font-semibold leading-snug">{title}</h4>
      {e ? (
        <EventBody e={e} fill={fill} />
      ) : f ? (
        <Markdown className="mt-1 text-[13px] leading-relaxed">{fill(f.body)}</Markdown>
      ) : null}
