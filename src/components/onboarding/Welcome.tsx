  ...
  return [
    {
      id: 'what',
      title: '무엇을 하는가',
      body: `실제로 일어난 금융위기 ${available.length}편 — ${span} — 을 자금·리스크 책임자, 정책당국, 펀드 운용역의 자리에서 다시 겪습니다. 각 시나리오는 공개된 감독 보고서와 1차 통계의 수치를 바탕으로 재구성했고, 화면에 나오는 모든 숫자는 출처를 따라갈 수 있습니다. 정답을 맞히는 퀴즈가 아니라, 불완전한 정보로 시간 안에 결정을 내리는 훈련입니다.`,
    },
    {
      id: 'how',
      title: '어떻게 진행되는가',
      body: `한 편은 ${turnRange}의 턴으로 이루어집니다. 위기가 격해지는 턴에서는 턴 안에서도 시각이 흐릅니다. 시계가 가는 동안 예금과 시장이 움직이고, 전화가 끼어들어 제한된 시간 안에 답을 요구하며, 결정에는 마감 시각이 있습니다. 감독관이나 인수 후보와는 여러 단계로 주고받으며 무엇을 얼마나 약속할지 고르고, 그 약속은 나중에 이행 여부로 평가됩니다. 확정하면 결과가 순서대로 펼쳐집니다. 브리핑에서 안내·표준·전문가 모드를 고를 수 있고, 한 편은 30~50분이면 끝납니다. 진행 상황은 이 브라우저에만 저장되므로 중간에 닫아도 이어서 할 수 있습니다.`,
    },
    {
      id: 'score',
      title: '무엇을 평가하는가',
      body: '6개 역량(유동성 관리·자본과 손실 관리·시장리스크 판단·위기 커뮤니케이션·규제와 거버넌스·거시 정책 판단)과 적시성을 합쳐 7개 차원으로 채점합니다. 점수는 선택한 옵션에 대한 전문가 평점과 실제 지표 결과를 함께 반영합니다. 디브리핑에서는 같은 시나리오를 실제 당사자가 택한 역사 경로, 전문가 권고 경로와 나란히 놓고 비교합니다.',
    },
  ]
}
...
export function Welcome({
  ...
  return (
    <section aria-labelledby="welcome-h" className="space-y-3">
      ...
      <ul className="grid list-none gap-3 p-0 m-0 md:grid-cols-3">
        {introCards().map((c) => (
          <li key={c.id}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Link
          to="/demo"
          className="inline-flex items-center gap-1.5 rounded-md border border-border-strong px-3 py-1.5 text-sm font-medium"
        >
          <Icon name="clock" size={14} />
          90초 둘러보기
        </Link>
        <span className="text-sm text-muted">한 턴을 네 단계로 미리 봅니다. 저장되지 않습니다.</span>
